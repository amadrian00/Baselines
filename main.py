...
def learn_framework(dataloaders, shape, train_labels, graph_name, fold, vis):
    if graph_name == 'proposed':
        pretrain = Pretrain(in_size=shape[-1], hidden_size= hidden_size, dropout=dropout,
                               device=device, y=train_labels, seq_len=0, num_layers=num_layers,
                            num_hyperedges=num_hyperedges, num_heads=num_heads, fold=fold).to(device)
        ...
        pretrain(next(iter(dataloaders['test'])).to(device))
        ...
        pretrain(next(iter(new_dataloaders['test'])).to(device))
    else:
        new_dataloaders = dataloaders
    hgl = FCHypergraphLearning(in_size=shape[-1], hidden_size=hidden_size, dropout=dropout,
                               device=device, y=train_labels, name=graph_name, num_hyperedges=num_hyperedges, fold=fold).to(device)
...
def k_folds():
    ...
    #for name in ['knn', 'k-random', 'gat', 'gcn', 'gsage', 'thfcn', 'ts-modelling', 'fc-modelling']:
    #for name in ['btf','knn', 'k-random', 'gat', 'gcn', 'gsage', 'thfcn', 'ts-modelling', 'fc-modelling']:
    for name in ['proposed']:
        for repeat, folds in enumerate(fold_batches):
            ...
            for fold, (train_loader, test_loader, val_loader, train_labels) in enumerate(folds):
                ...
                metrics = learn_framework(dataloaders, shape, train_labels, name, total_fold, vis)
                total_fold += 1
