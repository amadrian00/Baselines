...
class FCHypergraph(Dataset):
    def __init__(self, data, k=2, th = 0.05, torch_device='cpu', **kwargs):
        ...
        try:
            if os.path.exists('../../baseline_models/calculated_ts.pt'):
                self.calculated_ts = torch.load('../../baseline_models/calculated_ts.pt')
                self.loaded = True
            else:
    ...
    def generate_graphs(self):
        ...
        if not self.loaded: torch.save(self.calculated_ts, '../../baseline_models/calculated_ts.pt')
    ...
    def generate_graphs(self):
        ...
        for batch in self.data:
            for i, fc in enumerate(batch.x.view(-1,200,200)):
                ...
                graph = Data(x=proposed_x, y=batch.y[i],
