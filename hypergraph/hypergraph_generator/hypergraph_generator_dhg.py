...
from baseline_models.thfcn.construct_hyper_graph_KNN import construct_H_with_KNN
from baseline_models.thfcn.THFCN_ADNI import normalize
...
class FCHypergraph(Dataset):
    ...
    def generate_graphs(self):
        ...
        for i, matrix in enumerate(self.x):
            ...
            thfcn_hyperedge_index, thfcn_hyperdge_index = self.thfcn(self.data['time'][i][0])

            graph = Data(x=matrix, y=self.y[i].view(-1, 1),

                         eye=torch.eye(matrix.shape[0]),

                         knn=get_hypergraph(hyperedge_index, hyperedge_weights),

                         ts=get_hypergraph(ts_modelling_index, ts_modelling_weight),

                         fc=get_hypergraph(fc_modelling_index, fc_modelling_weight),

                         random=get_hypergraph(random_hyperedge_index, random_hyperedge_weight),

                         thfcn=get_hypergraph(thfcn_hyperedge_index, thfcn_hyperdge_index),
                         )
...
def get_hypergraph(edge_index, edge_weights):
    edge_index = [[int(edge_index[0, i].item()), int(edge_index[1, i].item())] for i in range(edge_index.shape[1])]
    return Hypergraph(200, edge_index, edge_weights.tolist())
...
class SecondFCHypergraph(Dataset):
    ...
    def generate_graphs(self):
        ...
        for batch in self.data:
            for i, fc in enumerate(batch.x.view(-1,200,200)):
                proposed_hyperedge_index, proposed_hyperedge_weight, proposed_x = self.proposed(fc.to('cuda'))

                graph = Data(x=proposed_x, y=batch.y[i].view(-1, 1),
                             proposed=get_hypergraph(proposed_hyperedge_index, proposed_hyperedge_weight),
                             )
    ...
    def proposed(self, fc):
        incidence, _, x, _ = self.model.forward(fc)


        incidence = incidence.detach().cpu().squeeze()

        proposed_hyperedge_index, weight = dense_to_sparse(incidence)
        return proposed_hyperedge_index, weight, x.detach().cpu()
